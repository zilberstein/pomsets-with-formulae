...
noncomputable def while_body [ScottCompact act] [ScottCompact test]
    (b : test) (p κ : Pom (Label act test)) : Pom (Label act test) :=
  if_stmt b (seq p κ) skip

lemma while_body_monotone [ScottCompact act] [ScottCompact test]
    (b : test) (p : Pom (Label act test)) : Monotone (while_body b p) := by
...
lemma while_body_continuous [ScottCompact act] [ScottCompact test]
...
noncomputable def while_loop [ScottCompact act] [ScottCompact test]
    (b : test) (p : Pom (Label act test)) : Pom (Label act test) :=
  lfp _ (while_body_monotone b p)

theorem lin_if_stmt [ScottCompact act] [ScottCompact test]
    (b : test) (p q : Pom (Label act test)) :
    (lin (if_stmt b p q) : st → t st) =
    fun σ ↦ Sem.sem b σ >>= fun r ↦ bif r then lin p σ else lin q σ :=
  lin_guard _ _ _

theorem lin_skip : @lin t st act test _ _ _ _ _ _ _ skip = pure := lin_fork
...
lemma lin_while_body [ScottCompact act] [ScottCompact test]
    (b : test) (p : Pom (Label act test)) (n : ℕ) :
...
theorem lin_while [ScottCompact act] [ScottCompact test]
    (b : test) (p : Pom (Label act test)) :
